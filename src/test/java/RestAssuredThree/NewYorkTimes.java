...
//		RestAssured.rootPath = "results[0]"; // Root Node From Where JSONPATH Will Be Start Applying
	...
	public void softAssert(){
			...
			ValidatableResponse response = given()
					...
					.spec(respSpec)
					.assertThat()
					.body("section", containsString(subject), // 1st Matcher
							"results.subsection", hasItem("asia"), // 2nd Matcher
							"results[0].subsection", containsString("dsfasfs"), // 3rd Matcher
							"results[0].subsection", containsString("asia")); // 4th Matcher

		// All The Matchers Are Asserted.
	}
